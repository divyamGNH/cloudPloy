...
A cloud-based deployment platform that automates building and deploying web applications from GitHub repositories. Submit a repository URL and CloudPloy handles cloning, building, containerization, and deployment—similar to Vercel or Heroku but built from scratch.

## Overview

CloudPloy implements a full deployment pipeline that transforms git repositories into containerized applications running on AWS infrastructure. The system demonstrates distributed system design, containerization, and cloud orchestration at scale.

### Core Features

- **Automated Build Pipeline**: End-to-end automation from git checkout to deployed container
- **Multi-stage Deployment**: Separate build orchestration and Docker image creation stages
- **AWS ECS Fargate Integration**: Serverless container orchestration with auto-scaling capabilities
- **Dynamic Subdomain Routing**: Multi-tenant deployment support with project-specific routing
- **Deployment Tracking**: ProjectID and DeploymentID separation enabling version history and rollback capability
- **AWS ECR Registry Integration**: Centralized Docker image management and versioning
- **Custom Build Commands**: Support for flexible build and start configurations per project
